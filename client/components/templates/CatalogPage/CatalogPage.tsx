...
import {
  ...
  $filteredTattooMachines,
  $machineManufacturers,
  $tattooMachines,
  setCountryManufacturers, setMachinesManufacturers,
  setTattooMachines, updateCountryManufacturer, updateMachineManufacturer
...
const CatalogPage = ({ query }: { query: IQueryParams }) => {
  ...
  const filteredTattooMachines = useStore($filteredTattooMachines)
  ...
  const pagesCount = Math.ceil(tattooMachines.count / 20)
  const isValidOffset =
  ...
  const resetFilterBtnDisabled = !(
    isPriceRangeChanged ||
    isAnyMachineManufacturerChecked ||
    isAnyCountryManufacturerChecked
  )
  const { toggleOpen, open, closePopup } = usePopup()

  useEffect(() => {
    loadTattooMachines()
  }, [filteredTattooMachines, isFilterInQuery])

  console.log(tattooMachines.rows)
    ...
    try {
      ...
      if (isValidOffset) {
        if (+query.offset > Math.ceil(data.count / 20)) {
          ...
          setCurrentPage(0)
          setTattooMachines(isFilterInQuery ? filteredTattooMachines : data)
          return
        }

        const offset = +query.offset - 1
        const result = await getTattooMachinesFx(
          `/tattoo-machines?limit=20&offset=${offset}`
        )

        setCurrentPage(offset)
        setTattooMachines(isFilterInQuery ? filteredTattooMachines : result)
        return
      }
  ...
  const resetPagination = (data: ITattooMachines) => {
    setCurrentPage(0)
  ...
  const handlePageChange = async ({ selected }: { selected: number }) => {
    try {
      ...
      if (selected > pagesCount) {
        resetPagination(isFilterInQuery ? filteredTattooMachines : data)
        return
      }
      ...
      const { isValidMachineQuery, isValidCountryQuery, isValidPriceQuery } =
        checkQueryParams(router)

      const result = await getTattooMachinesFx(
        `/tattoo-machines?limit=20&offset=${selected}${
          isFilterInQuery && isValidMachineQuery
            ? `&machine=${router.query.machine}`
            : ''
        }${
          isFilterInQuery && isValidCountryQuery
          ...
          <AnimatePresence>
            {isAnyMachineManufacturerChecked && (
              <ManufacturersBlock
                ...
                event={updateMachineManufacturer}
          ...
          <AnimatePresence>
            {isAnyCountryManufacturerChecked && (
              <ManufacturersBlock
                ...
                event={updateCountryManufacturer}
          ...
          <div className={styles.catalog__bottom__inner}>
            ...
            {spinner ? (
              ...
              <ul className={styles.catalog__list}>
                {tattooMachines.rows?.length ? (
                  tattooMachines.rows.map((item) => (
